...
class TestPickDataBlock:
    ...
    def test_system_keyword(self, proxy):
        result = proxy.pick_data_block("here is the system prompt", "")
        assert result is proxy.DATA_SYSTEM

    def test_generic_onboarding_falls_back_to_messaging(self, proxy):
        result = proxy.pick_data_block("helping with onboarding", "")
        assert result is proxy.DATA_MESSAGING

    def test_generic_setup_falls_back_to_messaging(self, proxy):
    ...
    def test_priority_messaging_over_generic(self, proxy):
