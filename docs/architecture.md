...
**Model parameters.** Temperature (`0.4`), context window (`2048` tokens), top-p (`0.85`), repeat penalty (`1.15`), and max prediction length (`200` tokens) are all configurable via `datura.env`. The low temperature and short prediction length keep responses focused and concise — the model is an assistant, not a storyteller.
